...
class Deferral(ValueStream):
    ...
    def precheck_failure(self, tstep, rte, sys_load, generation):
        ...
        Args:
            ...
            generation (list): total generation

        Returns:
            A list of minimum storage power (kw_rated) and energy (kwh_min) capacity required to meet deferral

        Notes:
            This algorithim can reliably find the last year defferal is possible, however the problem might still
    ...
    def objective_constraints(self, variables, subs, generation, reservations=None):
        ...
        constraints = []
        print(str(subs.index.year[-1])) if self.verbose else None
        if subs.index.year[-1] < self.year_failed:
            ...
            tot_load_deferred = subs['load'].values + self.load.loc[subs.index].values
