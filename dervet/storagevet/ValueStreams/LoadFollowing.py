...
class LoadFollowing(ValueStream):
    ...
    def objective_function(self, variables, subs, generation, annuity_scalar=1):
        ...
        p_lf_up = cvx.Parameter(subs.index.size, value=self.price_lf_up.loc[subs.index].values, name='price_lf_up')
        p_lf_do = cvx.Parameter(subs.index.size, value=self.price_lf_do.loc[subs.index].values, name='price_lf_do')
