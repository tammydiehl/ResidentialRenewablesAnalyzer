        ...
        for mo in months.unique():
            ...
            for per in pset:
                ...
                if np.all(billing_per_mask):
                    demand_charges += self.tariff.loc[per, 'Value'] * annuity_scalar * cvx.max(net_load)
                else:
                    demand_charges += self.tariff.loc[per, 'Value'] * annuity_scalar * cvx.max(net_load[billing_per_mask])
