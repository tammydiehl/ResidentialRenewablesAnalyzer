...
class Financial:
    ...
    def preform_cost_benefit_analysis(self, technologies, value_streams, results):
        ...
        if self.customer_sided:
            self.customer_bill(self.tariff, results.loc[:, 'Original Net Load (kW)'], results.loc[:, 'Net Load (kW)'])
    ...
    def calc_retail_energy_price(tariff, freq, analysis_yrs, non_zero=True):
        ...
        for year in analysis_yrs:
            ...
            temp_yr_indx = pd.date_range(start=first_day, end=last_day, freq=freq, closed='left')
            temp_yr_df = pd.DataFrame(index=temp_yr_indx)
            temp_yr_df.index.name = 'Start Datetime'

        temp = pd.concat([temp, temp_yr_df])
